...
namespace Editor
{
	...
	DvInspectorPanel::DvInspectorPanel(DvEditor* pEditor)
		: DvPanel(pEditor, "Inspector")
	{
		// 최소 크기 확보가 필요하다.
	...
	DvInspectorPanel::~DvInspectorPanel()
	{
	}

	void DvInspectorPanel::OnActiveScene(const Dive::Event& e)
	{
		//const auto& evt = dynamic_cast<const Dive::CreateSceneEvent&>(e);
		//m_pActiveScene = const_cast<Dive::Scene*>(evt.GetCreatedScene());
	}

	void DvInspectorPanel::renderWindow()
	{
		/*
		if (!m_pActiveScene)
			return;
