...
namespace Editor
{
	...
	DvHierarchyPanel::DvHierarchyPanel(DvEditor* pEditor)
		: DvPanel(pEditor, "Hierarchy")
	{
		//SUBSCRIBE_EVENT(eEventType::CreateScene, EVENT_HANDLER(OnActiveScene));
	}

	DvHierarchyPanel::~DvHierarchyPanel()
	{
	}

	void DvHierarchyPanel::OnActiveScene(const Dive::Event& e)
	{
		//const auto& evt = dynamic_cast<const Dive::CreateSceneEvent&>(e);
		//m_pActiveScene = const_cast<Dive::Scene*>(evt.GetCreatedScene());
	}

	void DvHierarchyPanel::renderWindow()
	{
		/*
		if (!m_pActiveScene)
		{
	...
	/*
	void DvHierarchyPanel::drawNode(Dive::GameObject* pObject)
	{
		if (!pObject)
			return;
