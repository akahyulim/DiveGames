...
#pragma pack(pop)
	...
	void Camera::Update()
	{
		...
		// ps constant buffer
		{
			...
			pMappedData->viewInverse = DirectX::XMMatrixTranspose(m_pGameObject->GetComponent<Transform>()->GetMatrix());
			m_pCBufferPS->Unmap();
		}
	}
	...
	{
		return DirectX::XMMatrixTranslationFromVector(m_pGameObject->GetComponent<Transform>()->GetPositionVector());
	}

	DirectX::XMMATRIX Camera::GetViewMatrix()
	{
		const auto& pos = m_pGameObject->GetComponent<Transform>()->GetPositionVector();
		const auto& forward = m_pGameObject->GetComponent<Transform>()->GetForwardVector();
		const auto& focus = DirectX::XMVectorAdd(pos, forward);

		const auto& up = m_pGameObject->GetComponent<Transform>()->GetUpwardVector();
