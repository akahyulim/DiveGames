...
namespace Dive
{
	IDXGISwapChain* GraphicsDevice::s_pSwapChain = nullptr;
	ID3D11Device* GraphicsDevice::s_pDevice = nullptr;
	ID3D11DeviceContext* GraphicsDevice::s_pDeviceContext = nullptr;

	ID3D11RenderTargetView* GraphicsDevice::s_pDefaultRenderTargetView = nullptr;
	ID3D11DepthStencilView* GraphicsDevice::s_pDefaultDepthStencilView = nullptr;
	ID3D11Texture2D* GraphicsDevice::s_pDefaultDepthTexture = nullptr;

	uint32_t GraphicsDevice::s_BackbufferWidth = 0;
	uint32_t GraphicsDevice::s_BackbufferHeight = 0;

	bool GraphicsDevice::s_bVSync = false;

	D3D11_PRIMITIVE_TOPOLOGY GraphicsDevice::s_PrimitiveTopology = D3D11_PRIMITIVE_TOPOLOGY_UNDEFINED;

	ID3D11RenderTargetView* GraphicsDevice::s_RenderTargetViews[4] = { nullptr, };
	ID3D11DepthStencilView* GraphicsDevice::s_pDepthStencilView = nullptr;

	bool GraphicsDevice::s_bRenderTargetViewsDirty = true;

	DvShader* GraphicsDevice::s_pShader = nullptr;

	DvVertexBuffer* GraphicsDevice::s_pVertexBuffer = nullptr;
	DvIndexBuffer* GraphicsDevice::s_pIndexBuffer = nullptr;

	ID3D11DepthStencilState* GraphicsDevice::s_pDepthStencilState = nullptr;
	ID3D11RasterizerState* GraphicsDevice::s_pRasterizerState = nullptr;
	ID3D11BlendState* GraphicsDevice::s_pBlendState = nullptr;

	ID3D11ShaderResourceView* GraphicsDevice::s_ShaderResourceViews[(uint32_t)eTextureUnit::Max_Num] = { nullptr, };
	ID3D11SamplerState* GraphicsDevice::s_SamplerStates[(uint32_t)eTextureUnit::Max_Num] = { nullptr, };

	bool GraphicsDevice::s_bTextureDirty = true;

	bool GraphicsDevice::Initialize(HWND hWnd, uint32_t width, uint32_t height, bool fullScreen)
	{
	...
	bool GraphicsDevice::UpdateSwapChain(uint32_t width, uint32_t height)
	{
	...
	void GraphicsDevice::ClearViews(uint8_t flags, const float* pColor, float depth, uint8_t stencil)
	{
	...
	void GraphicsDevice::SetRenderTargetView(uint32_t index, ID3D11RenderTargetView* pRenderTargetView)
	{
	...
	void GraphicsDevice::SetDepthStencilView(ID3D11DepthStencilView* pDepthStencilView)
	{
	...
	void GraphicsDevice::SetViewport(const RECT& rt)
	{
	...
	void GraphicsDevice::SetDepthStencilState(ID3D11DepthStencilState* pState)
	{
	...
	void GraphicsDevice::SetRasterizerState(ID3D11RasterizerState* pState)
	{
	...
	void GraphicsDevice::SetBlendState(ID3D11BlendState* pState)
	{
	...
	void GraphicsDevice::SetTexture(eTextureUnit index, DvTexture* pTexture)
	{
		if (pTexture)
		{
	...
	void GraphicsDevice::SetShader(DvShader* pShader)
	{
	...
	// 단일 버퍼만 다루고 있다.
	void GraphicsDevice::SetVertexBuffer(DvVertexBuffer* pBuffer)
	{
	...
	void GraphicsDevice::SetIndexBuffer(DvIndexBuffer* pBuffer)
	{
	...
	void GraphicsDevice::Draw(D3D11_PRIMITIVE_TOPOLOGY topology, uint32_t vertexCount, uint32_t vertexStart)
	{
		prepareDraw();
	...
	void GraphicsDevice::DrawIndexed(D3D11_PRIMITIVE_TOPOLOGY topology, uint32_t indexCount, uint32_t indexStart)
	{
		prepareDraw();
	...
	void GraphicsDevice::resetViews()
	{
