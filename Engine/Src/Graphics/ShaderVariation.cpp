...
namespace Dive
{
	...
	{
		DV_LOG_ENGINE_TRACE("ShaderVariation 소멸자 호출");
	}

	bool ShaderVariation::Compile(const std::string& filepath)
	{
		...
		{
			DV_LOG_ENGINE_ERROR("ShaderVariation::Compile - 전달받은 파일({:s})이 존재하지 않습니다.", filepath);
			return false;
		}
		...
		// 전체 이름도 붙여서 출력하기.
		if (m_Type == eShaderType::Vertex)
			DV_LOG_ENGINE_DEBUG("ShaderVariation::Compile - 정점 셰이더를 컴파일 하였습니다.");
		else
			DV_LOG_ENGINE_DEBUG("ShaderVariation::Compile - 픽셀 셰이더를 컴파일 하였습니다.");
		...
		if (m_Type == eShaderType::Vertex)
		{
			...
			DV_LOG_ENGINE_DEBUG("ShaderVariation::Compile - 정점 셰이더를 생성하였습니다.");
		}
		else
		{
			...
			DV_LOG_ENGINE_DEBUG("ShaderVariation::Compile - 픽셀 셰이더를 생성하였습니다.");
		}
