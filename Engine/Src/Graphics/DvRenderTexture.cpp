...
namespace Dive
{
	DvRenderTexture::DvRenderTexture()
		: DvTexture(eResourceType::DvRenderTexture),
	...
	bool DvRenderTexture::SetRenderTarget(int width, int height, DXGI_FORMAT format)
	{
		m_Width = width;
		m_Height = height;
		m_Format = format;
	...
	bool DvRenderTexture::SetDepthStencil(int width, int height, DXGI_FORMAT format, bool readOnly)
	{
		m_Width = width;
		m_Height = height;
		m_Format = format;
	...
	bool DvRenderTexture::createResources()
	{
		...
		{
			DV_RELEASE(m_pTexture2D);
			DV_CORE_ERROR("DvRenderTexture::createResources - Texture2D 생성에 실패하였습니다.");
			return false;
		}
		...
		{
			DV_RELEASE(m_pShaderResourceView);
			DV_CORE_ERROR("DvRenderTexture::createResources - ShaderResourceView 생성에 실패하였습니다.");
			return false;
		}
		...
		if (texDesc.BindFlags & D3D11_BIND_RENDER_TARGET)
		{
			...
			{
				DV_RELEASE(m_pRenderTargetView);
				DV_CORE_ERROR("DvRenderTexture::createResources - RenderTargetView 생성에 실패하였습니다.");
				return false;
			}
		}
		...
		if (texDesc.BindFlags & D3D11_BIND_DEPTH_STENCIL)
		{
			...
			if (FAILED(GraphicsDevice::GetDevice()->CreateDepthStencilView(
				static_cast<ID3D11Resource*>(m_pTexture2D),
				&dsvd,
				&m_pDepthStencilView)))
			{
				DV_CORE_ERROR("DvRenderTexture::createResources - DepthStencilView 생성에 실패하였습니다.");
				return false;
			}
			...
			if (m_bReadOnly)
			{
				...
				{
					DV_CORE_ERROR("DvRenderTexture::createResources - DepthStencilView ReadOnly 생성에 실패하였습니다.");
					return false;
				}
			}
		}
