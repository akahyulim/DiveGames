...
namespace Dive
{
	...
	bool DvTexture2D::SetImage(Image* pImage)
	{
		if (!pImage)
		{
			DV_CORE_ERROR("DvTexture2D::SetImage - 잘못된 Image를 전달받았습니다.");
			return false;
		}
	...
	void DvTexture2D::SetPixels(const std::vector<DirectX::XMINT4>& colors, int mipLevel)
	{
		...
		if (FAILED(GraphicsDevice::GetDeviceContext()->Map(
			...
			&mappedResource)))
		{
			DV_CORE_ERROR("DvTexture2D::SetRawTextureData - 텍스쳐 맵에 실패하였습니다.");
			return;
		}
	...
	bool DvTexture2D::SetRawTextureData(const void* pData)
	{
		if (!m_pTexture2D)
		{
			DV_CORE_ERROR("DvTexture2D::SetRawTextureData - 텍스쳐가 존재하지 않아 데이터를 전달할 수 없습니다.");
			return false;
		}

		if (!pData)
		{
			DV_CORE_ERROR("DvTexture2D::SetRawTextureData - 비어있는 데이터를 전달받았습니다.");
			return false;
		}
	...
	*/
	bool DvTexture2D::createResources()
	{
		...
		{
			DV_RELEASE(m_pTexture2D);
			DV_CORE_ERROR("DvTexture2D::createResources - DvTexture2D 생성에 실패하였습니다.");
			return false;
		}
		...
		{
			DV_RELEASE(m_pShaderResourceView);
			DV_CORE_ERROR("DvTexture2D::createResources - ShaderResourceView 생성에 실패하였습니다.");
			return false;
		}
		...
		if(texDesc.BindFlags & D3D11_BIND_RENDER_TARGET)
		{
			...
			{
				DV_RELEASE(m_pRenderTargetView);
				DV_CORE_ERROR("DvTexture2D::createResources - RenderTargetView 생성에 실패하였습니다.");
				return false;
			}
		}
