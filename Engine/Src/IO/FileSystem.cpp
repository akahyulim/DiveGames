...
namespace Dive
{
	...
	bool FileSystem::Copy(const std::string& sourceFileName, const std::string& destFileName)
	{
		...
		{
			DV_LOG(FileSystem, err, "원본 파일({:s})이 존재하지 않아 복사를 수행할 수 없습니다.", sourceFileName);
			return false;
		}

		if (sourceFileName == destFileName)
		{
			DV_LOG(FileSystem, warn, "동일한 파일 이름을 전달받아 복사를 수행할 수 없습니다. : {0:s} == {:1s}",
	...
	bool FileSystem::Rename(const std::string& sourceFileName, const std::string& destFileName)
	{
		if (!FileExists(sourceFileName))
		{
			DV_LOG(FileSystem, err, "원본 파일({:s})이 존재하지 않아 이름을 변경할 수 없습니다.", sourceFileName);
			return false;
		}

		if (destFileName.empty())
		{
			DV_LOG(FileSystem, err, "파일의 새로운 이름({:s})을 잘못 전달받아 변경할 수 없습니다.", destFileName);
			return false;
		}
