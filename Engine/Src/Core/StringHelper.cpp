...
namespace Dive
{
	namespace StringHelper
	{
		...
		// ---------------------------------------------------------------------------------------------------------
		std::string GetStringAfterToken(const char * str, const char * targetToken)
		{
			int tokenSize = static_cast<int>(strlen(targetToken));
			char cmpStr[256];
			std::string afterStr;
			...
			while (*str != '\0')
			{
				if (*str == *targetToken)
				{
					...
					if (0 == strcmp(cmpStr, targetToken))
						afterStr = str;
				}
		...
		std::string ReplaceToken(const char * str, const char * targetToken, const char * newToken)
		{
			int tokenSize = static_cast<int>(strlen(targetToken));
			char cmpStr[256];
			std::string fixedStr;
