...
#define DV_STRINGIFY_MACRO(x) #x
...
#ifdef _DEBUG
	#define DV_INTERNAL_ASSERT_IMPL(type, check, msg, ...) { if(!(check)) { DV##type##ERROR(msg, __VA_ARGS__); __debugbreak(); } }
	...
	#define DV_ENGINE_ASSERT(...) DV_EXPAND_MACRO( DV_INTERNAL_ASSERT_GET_MACRO(__VA_ARGS__)(_ENGINE_, __VA_ARGS__) )
#else
	#define DV_ASSERT(...)
	#define DV_ENGINE_ASSERT(...)
#endif