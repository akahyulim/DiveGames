...
namespace Dive
{
	...
	uint32_t Texture::CalMipMaxLevel(uint32_t width, uint32_t height)
	{
		uint32_t maxLevel = 1;
	...
	uint32_t Texture::GetLevelWidth(uint32_t level) const
	{
	...
	uint32_t Texture::GetLevelHeight(uint32_t level) const
	{
		if (level >= m_MipLevels)
			return 0;
	...
	uint32_t Texture::GetRowDataSize(uint32_t width) const
	{
		switch (m_Format)
		{
