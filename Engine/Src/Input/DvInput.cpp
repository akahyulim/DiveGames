...
namespace Dive
{
	...
	bool DvInput::Initialize(HINSTANCE hInstance, HWND hWnd)
	{
	...
	void DvInput::ReadMouse()
	{
		...
		/*
		// 추후 이벤트를 통해 갱신하자.
		DirectX::XMFLOAT2 windowSize = DvRenderer::GetResolutionRender();
	...
	bool DvInput::KeyState(int key)
	{
		return s_KeyStates[key] & 0X80;
	}

	bool DvInput::KeyDown(int key)
	{
		return (s_KeyStates[key] & 0X80) && !(s_OldKeyStates[key] & 0X80);
	}

	bool DvInput::KeyUp(int key)
	{
		return !(s_KeyStates[key] & 0X80) && (s_OldKeyStates[key] & 0X80);
	}

	bool DvInput::KeyPress(int key)
	{
		return (s_KeyStates[key] & 0X80) && (s_OldKeyStates[key] & 0X80);
	}

	bool DvInput::MouseButtonState(int btn)
	{
		return s_MouseButtons[btn] == 1;
	}

	bool DvInput::MouseButtonDown(int btn)
	{
		return (s_MouseButtons[btn] == 1) && !(s_OldMouseButtons[btn] == 1);
	}

	bool DvInput::MouseButtonUp(int btn)
	{
		return !(s_MouseButtons[btn] == 1) && (s_OldMouseButtons[btn] == 1);
	}

	bool DvInput::MouseButtonPress(int btn)
	{
		return (s_MouseButtons[btn] == 1) && (s_OldMouseButtons[btn] == 1);
	}

	const DirectX::XMFLOAT2& DvInput::GetMouseMoveDelta()
	{
		return s_MouseDelta;
	}

	const DirectX::XMFLOAT2& DvInput::GetMousePosition()
	{
		return s_MousePosition;
	}

	bool DvInput::GetMouseIsInValidRect()
	{
		return s_MouseIsInValidRect;
	}
	
	void DvInput::SetMouseIsInValidRect(bool valid)
	{
